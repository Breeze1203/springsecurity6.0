    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (!EXCLUDED_PATHS.contains(request.getRequestURI())) {
            ...
            if (!StringUtils.hasText(jwt)) {
                AuthFailure.getInstance().onAuthenticationFailure(request, response, new BadCredentialsException("令牌不存在"));
                return;
            }
            try {
                ...
                AuthFailure.getInstance().onAuthenticationFailure(request, response, new BadCredentialsException(e.getMessage()));
                return;
            }
        }
        // 继续过滤链
