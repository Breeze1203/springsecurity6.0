 ...
 */
public class CustomizeCsrfTokenRequestHandler extends CsrfTokenRequestAttributeHandler {
    ...
    /*
### CustomizeCsrfTokenRequestHandler
...
### 调用顺序
1. **CustomizeCsrfTokenRepository** 的 `loadToken` 方法首先被调用，以从存储中加载现有的CSRF令牌。
2. 如果需要生成新的CSRF令牌（例如，用户访问了一个需要认证的页面），则 **CustomizeCsrfTokenRepository** 的 `generateToken` 方法会被调用。
3. 接着，**CustomizeCsrfTokenRepository** 的 `saveToken` 方法会被调用，以保存新生成的令牌。
4. 当响应被发送回客户端之前，**CustomizeCsrfTokenRequestHandler** 的 `handle` 方法被调用，将CSRF令牌添加到响应头中。
5. 对于需要CSRF验证的请求，**CustomizeCsrfTokenRequestHandler** 的 `resolveCsrfTokenValue` 方法会在请求处理时被调用，以解析请求中的CSRF令牌值。

通过这种机制，Spring Security确保了CSRF令牌在客户端和服务器之间安全地传输和验证。`CustomizeCsrfTokenRepository`主要负责令牌的生命周期管理，而`CustomizeCsrfTokenRequestHandler`负责在请求处理过程中使用这些令牌。两者的协同工作确保了CSRF保护的有效实施。
